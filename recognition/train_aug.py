...
# Modified by USDA Agricultural Research Service in 2026
# Modifications are to v1.0.0 7dabbe1 of the file https://github.com/mindee/doctr/blob/main/references/recognition/train.py
# add image distortions from the ALbumentations library to improve model generalization
# all changes are covered under CC0 Public domain attribution 
...
try:
    ...
    raise ImportError("albumentations is required. Install it with: pip install albumentations")
...
class AlbumentationsBatchTransform:
    """Wrapper to apply Albumentations transforms to batches of torch tensors.
    
    Handles conversion between torch tensors and numpy arrays for Albumentations,
    and applies normalization after augmentation.
    """
    
    def __init__(self, aug_pipeline, normalize_mean=(0.694, 0.695, 0.693), 
                 normalize_std=(0.299, 0.296, 0.301), is_training=True):
        """
        Args:
            aug_pipeline: Albumentations Compose pipeline (or None for validation)
            normalize_mean: Mean for normalization
            normalize_std: Std for normalization
            is_training: If True, apply augmentations; if False, only normalize
        """
    ...
    def __call__(self, batch):
        ...
        Args:
            batch: Torch tensor of shape (B, C, H, W) with values in [0, 1]
        
        Returns:
            Normalized torch tensor
        """
        # Convert batch to numpy for Albumentations
        # batch shape: (B, C, H, W) -> need (B, H, W, C) for albumentations
        ...
        if self.is_training and self.aug_pipeline is not None:
            augmented_batch = []
            for img in batch_np:
                # Handle grayscale images (H, W) -> convert to (H, W, 1) for albumentations
                if len(img.shape) == 2:
                    img = np.expand_dims(img, axis=-1)
                
                # Apply augmentation
                augmented = self.aug_pipeline(image=img)
                augmented_batch.append(augmented['image'])
            batch_np = np.stack(augmented_batch, axis=0)
        
        # Convert back to torch tensor
        # batch_np shape: (B, H, W, C) -> (B, C, H, W)
        batch_torch = torch.from_numpy(batch_np).permute(0, 3, 1, 2).float()
        ...
        if batch.is_cuda:
            batch_torch = batch_torch.to(batch.device)
        
        # Apply normalization
        batch_torch = self.normalize(batch_torch)
        
        return batch_torch


def create_albumentations_pipeline(input_size):
    """Create Albumentations augmentation pipeline for training.
    
    Args:
        input_size: Height of input images (width = 4 * input_size)
    
    Returns:
        Albumentations Compose pipeline
    """
    return A.Compose([
        A.Resize(height=input_size, width=4 * input_size),
        ...
        A.Perspective(scale=(0.05, 0.1), fit_output=True, p=0.3),
        # CRITICAL: Forces exact batch size after geometric warp
        A.PadIfNeeded(
            min_height=input_size,
            min_width=4 * input_size,
            position='top_left',
