# Creating a real seven segment training dataset for OCR model training

Most OCR models work pretty poorly for seven segment displays. For example, EAsyOCR is a 
...
The best OCR training should come from training on images from our actual imaging setup. To simulate reading the red displays at the dairy we have purchased a 7 segment LED display that we can control. Using a Raspberry Pi that can control both our camera and the serial display we can gather a large number of images in sunlight and shade that  closely mimic the actual conditions in the dairy.

### Hardware

1. Raspberry Pi 4 Model B mounded in a [SmartPi Touch 2](https://smarticase.com/products/smartipi-touch-2) with a 7" display
...
We attached the lens to one end of a lab cart and the Raspberry Pi to the other end with the seven segment display attached to it at a distance of about 90 cm.  The display height was about 160 pixels out of 1088. The height of the dairy display which will be about 3 m away so it is a smaller image. We aquires thes inside near a window
...
Using this setup we collected 202,208 images in  and a CSV with file names and labels in test.csv`.
